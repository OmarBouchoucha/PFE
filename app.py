...
NOW_in_days = datetime.now().strftime('%Y-%m-%d')
...
def analytics():
    ...
    for i in range(len(data)):
        ...
        hums.append(data[i].humidity)
        weis.append(data[i].weight)
        tims.append(data[i].time)
    plot(temps,tims,"tempature",'Sensor Tempature',"tempature")
    plot(hums,tims,"humidity",'Sensor Humidity',"humidity")
    plot(weis,tims,"weight",'Sensore Weight',"weight")
...
def customintervales():
    if request.method =="POST":
        ...
        for i in range(len(inp)):
            time = inp[i].time
            date = inp[i].date    
            if not(NOW_in_hours > time and  NOW_in_days >= date):   
                inp[i].time = "canceled"
                inp[i].date = "canceled"
                db.session.commit()
        name = session["name"]
    ...
    if request.method =="POST":
        ...
        for i in range(len(inp)):
            ...
            if not(NOW_in_hours > time and  NOW_in_days >= date):   
                ...
                inp[i].date = "canceled"
                db.session.commit()
        report = reports_db(report=f"Started drying a {fruit} ",date=NOW_in_days,time=NOW_in_hours,user=found_user)
        db.session.add(report)  
        db.session.commit()
        return redirect(url_for("command"))
...
def fruitgraph():
    ...
    for i in range(len(tempatures)):
        if tempatures[i].fruit.name == fruit:
            id = fruit_instance.id
    tem = temperature_db.query.filter_by(fruit_id=id).all()
    dur = duration_db.query.filter_by(fruit_id=id).all()
    ...
    plot(temps,durs,"tempature",f"Display of the values","fruit")
